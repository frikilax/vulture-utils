...
usage() {
    ...
    echo "	-b	Use a Boot Environment to install updates, and activate for next boot"
    echo "	-B	Use a Boot Environment to install updates, and validate for all future reboots"
    echo "	-k <num>	Number of snapshots/BEs to keep (default is 2)"
...
update_system() {
    ...
    if [ -f /usr/sbin/hbsd-update ] ; then
        ...
        if [ -z "$jail" ] && [ $snapshot_system -gt 0 ]; then
...
# parse parameters #
####################
while getopts 'hDTV:cdusbBk:t:r:' opt; do
    case "${opt}" in
        D)  download_only=1;
            keep_temp_dir=1;
            ;;
...
if [ $snapshot_system -gt 0 ] && [ $do_update_packages -gt 0 ]; then
    warn "snapshotting asked, won't upgrade packages"
    do_update_packages=0
fi
...
for jail in "haproxy" "redis" "mongodb" "rsyslog" ; do
    if [ -z "$1" -o "$1" == "$jail" ] ; then
        ...
        if [ $do_update_system -gt 0 ]; then
            ...
            download_system_update || finalize 1 "Failed to download system upgrades for jail ${jail}"
            update_system "$jail" || finalize 1 "Failed to install system upgrades in jail ${jail}"
            echo "[-] Ok."
        fi
...
if [ -z "$1" -o "$1" == "gui" ] ; then
    ...
    if [ $do_update_system -gt 0 ]; then
        ...
        download_system_update || finalize 1 "Failed to download system upgrades for jail apache"
        update_system "apache" || finalize 1 "Failed to install system upgrades in jail apache"
        echo "[-] Ok."
        echo "[+] Updating jail portal base system files..."
        download_system_update || finalize 1 "Failed to download system upgrades for jail portal"
        update_system "portal" || finalize 1 "Failed to install system upgrades in jail portal"
        echo "[-] Ok."
    fi
